...
impl<'input> fmt::Display for Token<'input> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ...
            Token::Regex(s, flags) => write!(f, "/{}/{}", s, flags),
            ...
            Token::In => write!(f, "in"),
            Token::Matches => write!(f, "~"),
            Token::Capture => write!(f, "@"),
            Token::CaptureMultiple => write!(f, "@+"),
            Token::And => write!(f, "&&"),
            Token::Or => write!(f, "||"),
            ...
            Token::DollarNow => write!(f, "$now"),
            Token::DollarNowUtc => write!(f, "$now_utc"),
        }
    }
}
...
impl<'input> Iterator for Lexer<'input> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        match self.next_token_after_whitespace() {
            Ok(token) => {
                // Store the token for context in next iteration
                self.last_token = Some(token);
                Some(Ok((start_pos, token, self.position)))
            }
...
impl<'input> Lexer<'input> {
        ...
        match ch {
            ...
            '.' => {
                if self.position + 1 < self.input.len() {
                    let next_char = self.input[self.position + 1..].chars().next().unwrap_or('\0');
            ...
            // Identifiers, keywords, and $now / $now_utc
            '$' => {
                self.advance();
                    ...
                    if self.current_char() == 'o' {
                        ...
                        if self.current_char() == 'w' {
                            ...
                            if !self.is_at_end() && self.current_char() == '_' {
                                self.advance(); // _
                                if !self.is_at_end() && self.current_char() == 'u' {
                                    self.advance(); // u
                                    if !self.is_at_end() && self.current_char() == 't' {
                                        ...
                                        if !self.is_at_end() && self.current_char() == 'c' {
                                            ...
                                            {
                                                Err(LexError {
                                                    message: "Invalid identifier starting with '$'".to_string(),
                                                    line: self.line,
                                                    column: self.column,
                                                })
                                            } else {
                                                Ok(Token::DollarNowUtc)
                                            }
                                        } else {
                                            Err(LexError {
                                                message: "Invalid identifier starting with '$'".to_string(),
                                ...
                                && (self.current_char().is_alphanumeric())
                            {
                                // $now followed by alphanumeric (but not _utc)
                                Err(LexError {
                                    message: "Invalid identifier starting with '$'".to_string(),
                                    line: self.line,
                                    column: self.column,
    ...
    fn scan_regex(&mut self) -> Result<Token<'input>, LexError> {
        ...
        while !self.is_at_end() {
            ...
            if ch == '/' {
                ...
                let regex_slice = &self.input[start_pos..end_pos];

                // Consume optional flags (e.g. 'i' for case-insensitive)
                let flags_start = self.position;
                while !self.is_at_end() && self.current_char().is_ascii_alphabetic() {
                    self.advance();
                }
                let flags_slice = &self.input[flags_start..self.position];

                return Ok(Token::Regex(regex_slice, flags_slice));
            } else if ch == '\\' {
                ...
                if !self.is_at_end() && self.current_char() == '\'' {
                    ...
                    self.advance(); // consume any escaped character
                } else {
                    return Err(LexError {
    ...
    fn scan_number(&mut self) -> Result<Token<'input>, LexError> {
        ...
        if !&self.input[start_pos..self.position].starts_with('.')
            && !&self.input[start_pos..self.position].starts_with("-.")
            && !self.is_at_end()
            && self.current_char() == '.'
        {
    ...
    // Check if context suggests `/` should be a regex rather than division
    fn expect_regex(&self) -> bool {
        // Check if the next character (after /) looks like it could start a regex pattern
        let next_pos = self.position + 1;
        if next_pos < self.input.len() {
            // Get the next character properly handling UTF-8
            if let Some(next_char) = self.input[next_pos..].chars().next() {
                let looks_like_regex = next_char.is_alphanumeric()
                    || next_char == '\\'
                    || next_char == '['
                    || next_char == '.'
                    || next_char == '^'
                ...
                if !looks_like_regex {
                    return false; // Definitely not a regex
                }
            } else {
                return false; // Invalid UTF-8 or end
            }
        } else {
            return false; // End of input, can't be regex
        }

        match &self.last_token {
            None => true, // At start of input with regex-like pattern
            Some(token) => matches!(
                token,
                // After operators, `/` is likely a regex
                Token::Plus | Token::Minus | Token::Multiply | Token::Divide |
                Token::FloorDivide | Token::Modulus | Token::Exponent |
                Token::Equal | Token::NotEqual | Token::Greater | Token::GreaterEqual |
                Token::Less | Token::LessEqual | Token::And | Token::Or |
                Token::Matches | Token::Capture | Token::CaptureMultiple |
                // After delimiters
                Token::LeftParen | Token::LeftBracket | Token::LeftBrace |
                Token::Comma | Token::Colon | Token::Pipe | Token::Question |
                // After keywords
                Token::If | Token::Else | Token::In | Token::Not
            ),
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn test_numbers() {
        let lexer = Lexer::new("123 45.67 .89 -10");
        let tokens: Result<Vec<_>, _> = lexer.collect();
        let tokens = tokens.unwrap();
    ...
    fn test_identifiers_and_keywords() {
        let lexer = Lexer::new("foo true false null in if else");
        let tokens: Result<Vec<_>, _> = lexer.collect();
        let tokens = tokens.unwrap();
    ...
    #[test]
    fn test_comments() {
        let lexer = Lexer::new("1 + 2 # this is a comment\n3 + 4");
        let tokens: Result<Vec<_>, _> = lexer.collect();
        let tokens = tokens.unwrap();
    ...
    #[test]
    fn test_regex() {
        let lexer = Lexer::new(r#" /\w+/ "#);
        let tokens: Result<Vec<_>, _> = lexer.collect();
        let tokens = tokens.unwrap();

        let expected_tokens = vec![Token::Regex("\\w+", "")];
    ...
    #[test]
    fn test_regex_flags() {
        let lexer = Lexer::new(r#" /will/i "#);
        let tokens: Result<Vec<_>, _> = lexer.collect();
        let tokens = tokens.unwrap();

        let expected_tokens = vec![Token::Regex("will", "i")];
    ...
    fn test_dollar_now() {
        ...
        let expected_tokens = vec![Token::DollarNow];
    ...
    #[test]
    fn test_dollar_now_utc() {
        let lexer = Lexer::new("$now_utc");
        let tokens: Result<Vec<_>, _> = lexer.collect();
        let tokens = tokens.unwrap();

        let expected_tokens = vec![Token::DollarNowUtc];
